...
func NewPagination(params ...interface{}) *Pagination {
	if len(params) >= 1 {
		var config *Config
...
func createCauses(p pageRequest) requestQuery {
	...
	for _, so := range p.Sorts {
		originalColumn := so.Column

		if strings.Contains(originalColumn, ".") {
			parts := strings.Split(originalColumn, ".")
			columnName := parts[len(parts)-1]

			so.Column = "s." + columnName
		} else {
			so.Column = "s." + originalColumn
		}
...
type Config struct {
	...
	CacheAdapter         *AdapterInterface                      `json:"-"`
